...
function updateStat(type, available, total) {

  const statItem = document.querySelector(`.stat-icon.${type}`)
                  .closest(".stat-item");

  const statVal = statItem.querySelector(".stat-val");
  const statBar = statItem.querySelector(".stat-bar");

  statVal.innerHTML = `${available}<small>/${total}</small>`;
